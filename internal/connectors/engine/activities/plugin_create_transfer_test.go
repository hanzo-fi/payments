...
var _ = Describe("Plugin Create Transfer", func() {
	...
	Context("plugin create transfer", func() {
		var (
		...
		It("calls underlying plugin", func(ctx SpecContext) {
			...
			plugin.EXPECT().CreateTransfer(ctx, req.Req).Return(sampleResponse, nil)
			res, err := act.PluginCreateTransfer(ctx, req)
			Expect(err).To(BeNil())
		...
		It("returns a retryable temporal error", func(ctx SpecContext) {
			p.EXPECT().Get(req.ConnectorID).Return(plugin, nil)
			plugin.EXPECT().CreateTransfer(ctx, req.Req).Return(sampleResponse, fmt.Errorf("some string"))
