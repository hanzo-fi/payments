...
func paymentsList(backend backend.Backend) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		query, err := paginate.Extract[storage.ListPaymentsQuery](r, func() (*storage.ListPaymentsQuery, error) {
		...
		for i := range cursor.Data {
			data[i] = &PaymentResponse{
				...
				Scheme:        toV2PaymentScheme(cursor.Data[i].Scheme),
		...
		err = json.NewEncoder(w).Encode(api.BaseResponse[*PaymentResponse]{
			Cursor: &paginate.Cursor[*PaymentResponse]{
