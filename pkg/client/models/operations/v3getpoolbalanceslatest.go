// Code generated by Speakeasy (https://speakeasy.com). DO NOT EDIT.

package operations

import (
	"github.com/hanzo-fi/payments/pkg/client/models/components"
)

type V3GetPoolBalancesLatestRequest struct {
	// The pool ID
	PoolID string `pathParam:"style=simple,explode=false,name=poolID"`
}

func (o *V3GetPoolBalancesLatestRequest) GetPoolID() string {
	if o == nil {
		return ""
	}
	return o.PoolID
}

type V3GetPoolBalancesLatestResponse struct {
	HTTPMeta components.HTTPMetadata `json:"-"`
	// OK
	V3PoolBalancesResponse *components.V3PoolBalancesResponse
	// Error
	V3ErrorResponse *components.V3ErrorResponse
}

func (o *V3GetPoolBalancesLatestResponse) GetHTTPMeta() components.HTTPMetadata {
	if o == nil {
		return components.HTTPMetadata{}
	}
	return o.HTTPMeta
}

func (o *V3GetPoolBalancesLatestResponse) GetV3PoolBalancesResponse() *components.V3PoolBalancesResponse {
	if o == nil {
		return nil
	}
	return o.V3PoolBalancesResponse
}

func (o *V3GetPoolBalancesLatestResponse) GetV3ErrorResponse() *components.V3ErrorResponse {
	if o == nil {
		return nil
	}
	return o.V3ErrorResponse
}
